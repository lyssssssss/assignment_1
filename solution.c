...
int main(int argc, char* argv[]) {
    ...
    for (step = 1; step < size; step <<= 1) {
        if (rank % (2 * step) == 0) {
            if (rank + step < size) {
                long incoming_cost_per_menu_item[ITEMS_ON_MENU];
                MPI_Recv(incoming_cost_per_menu_item, ITEMS_ON_MENU, MPI_LONG, rank + step, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                for (int i = 0; i < ITEMS_ON_MENU; i++) {
                    cost_per_menu_item[i] += incoming_cost_per_menu_item[i];
                }
            }
        } else {
    ...
    if (rank == ROOT_PROCESS) {
        ...
        for (int i = 0; i < ITEMS_ON_MENU; i++) {
            total_cost_per_menu_item[i] = cost_per_menu_item[i];
        }
    }
