...
int main(int argc, char* argv[]) {
    ...
    // Perform tree-based reduction to sum up costs in a logarithmic number of steps
    int step = 1;
    while (step < size) {
        if ((rank % (2 * step)) == 0) {
            if (rank + step < size) {
                long buffer[ITEMS_ON_MENU];
                MPI_Recv(buffer, ITEMS_ON_MENU, MPI_LONG, rank + step, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                for (int i = 0; i < ITEMS_ON_MENU; i++) {
                    cost_per_menu_item[i] += buffer[i];
                }
            }
        } else {
            int target = rank - (rank % step);
    ...
    if (rank == 0) {
        memcpy(total_cost_per_menu_item, cost_per_menu_item, ITEMS_ON_MENU * sizeof(long));
    }
